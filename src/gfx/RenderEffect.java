...
public class RenderEffect {
	...
	public static BufferedImage screenDistortion(BufferedImage image) {
		for(int y = 0; y < image.getHeight(); y++) {
			int distanceFromCenter = Math.abs((image.getHeight() / 2) - y);
			int xPixel = (int)((1 / 30.0) * distanceFromCenter);
			System.out.println(xPixel);
			for(int x = xPixel; x >= 0; x--) {
				image.setRGB(0 + x, y, Color.BLACK.getRGB());
				image.setRGB(image.getWidth() - 1 - x, y, Color.BLACK.getRGB());
			}
		}
		return image;
	}
