...
public abstract class Creature extends Entity {
	public static final int invulnerabilityFrames = 45;
	protected int currentInvulnerableFrames = invulnerabilityFrames;

	protected int health, maxHealth;
	protected double speed;
	protected double yVelocity;
	protected boolean dying, jumping, falling, attacking, invulnerable, hitFromRight, stunned;
	...
	protected void moveX(Entity.Direction direction, double moveSpeed) {
		if(direction == Entity.Direction.RIGHT) {
			if(checkMapCollision((int)(x + moveSpeed), (int)y, (int)x, (int)y)) {
				x += moveSpeed;
			} else {
	...
	protected void moveY(Entity.Direction direction, double moveSpeed) {
		if(direction == Entity.Direction.UP) {
			if(checkMapCollision((int)x, (int)(y - moveSpeed), (int)x, (int)y)) {
				y -= moveSpeed;
			} else {
	...
	public abstract Entity clone(boolean sideFacing);
	...
	public void draw(Graphics2D g, Camera camera) {
		if(dying) {
			drawSoul(g, camera);
		} else if(camera.inBounds((int)x, (int)y)) {
			if(facingRight)
